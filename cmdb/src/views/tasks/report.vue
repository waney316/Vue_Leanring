<template>
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix report-title">
        <span>任务ID: {{ task_id }}</span>
        <span>巡检时间: {{ task.create_time }}</span>
        <span>
          任务状态:
          <el-tag type="success" style="display: inline">{{
            task.task_status
          }}</el-tag>
        </span>
      ...
      <div v-for="item in list" :key="item.id" class="text item">
        <el-collapse accordion>
          <el-collapse-item :title="item.ip" name="1">
            <el-row>
              <el-col :span="12"
                ><span>关联服务:</span
                ><el-tag
...
export default {
  ...
  created() {
    this.task = JSON.parse(this.$route.query.row);
